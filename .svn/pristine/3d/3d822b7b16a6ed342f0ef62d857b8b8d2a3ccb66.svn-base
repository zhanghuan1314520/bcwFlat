...
Page({
  ...
  regist: function (e) {
    ...
    if (that.data.phonenumber == '') {
      ...
      wx.request({
        ...
        // url: axiosurl + '/bcapi/api/ipad/padManageUserParent/register?localPhone=' + that.data.phonenumber + "&password=" + that.data.password + "&passwordAgain=" + that.data.passwordack + "&verifyCode=" + that.data.code,
        url: axiosurl + '/bcapi/api/ipad/padManageUserParent/register',
        data:{
  ...
  logins: function (e) {
    ...
    //   判断是同意结束
      ...
      wx.request({
        url: axiosurl + '/bcapi/api/ipad/padManageUserParent/login',
        method: "POST",
        data:{
  ...
  // /api/sms/sendVerifyCode  获取 短信验证码
  // 参数：mobile ：手机号，companyType=2（固定死值）
  // 验证码不正确的错误码是1605
